...
# Background (environment) temperature in K
T_env = 2.7  # ~ cosmic microwave background
...
M      = np.zeros(5)
cp     = np.zeros(5)
alpha  = np.zeros(5)  # absorptivity (for Node 1 if it sees sun)
epsilon= np.zeros(5)  # emissivity (for nodes radiating to space)
...
for step_i in range(1, n_steps):
    ...
    # --- Node 1 (Solar Cell) ---
    A1_sun = 1.0  # area seeing the sun
    Q_solar_1 = alpha[0] * A1_sun * solar_flux

    # Radiative flux to a 2.7 K background
    # net = e*sigma*A*(T^4 - T_env^4)
